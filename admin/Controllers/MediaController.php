...
class MediaController extends BaseController
{
            ...
            if ($file->isValid()) {
                ...
                if (!in_array($ext, $this->mediaConfig->allowedExtensions)) {
                    return response()
                        ->setStatusCode(422)
                        ->setJSON([
                            'errors' => [lang('admin.media.filetype_not_allowed')]
                        ]);
                }
                ...
                if (!in_array($mime, $this->mediaConfig->allowedMimeTypes)) {
                    return response()
                        ->setStatusCode(422)
                        ->setJSON([
                            'errors' => [lang('admin.media.mimetype_not_allowed')]
                        ]);
                }
                ...
                try {
                    ...
                    return response()->setJSON([
                        'message' => lang('admin.media.upload_success'),
                        'name' => $file->getName()
                    ]);
        ...
        if ($this->request->getMethod() === 'POST') {
            ...
            if (!$filename || !file_exists(WRITEPATH . 'media/' . $path . $filename)) {
                return response()->setStatusCode(404)
                    ->setJSON([
                        'errors' => ['not found' => lang('admin.media.file_not_exist')]
                    ]);
            }
            ...
            if (!unlink(WRITEPATH . 'media/' . $path . $filename)) {
                return $this->response->setStatusCode(500)
                    ->setJSON([
                        'errors' => ['delete' => lang('admin.media.file_delete_error')]
                    ]);
            }

            return response()->setJSON([
                'message' => lang('admin.media.file_delete_success')
            ]);
        }
    ...
    public function removeDir(): ResponseInterface
    {
        if ($this->request->getMethod() === 'POST') {
            ...
            if (!$dir || !is_dir(WRITEPATH . 'media/' . $path . $dir)) {
                return response()->setStatusCode(404)
                    ->setJSON([
                        'errors' => ['not found' => lang('admin.media.dir_not_exist')]
                    ]);
            }
            ...
            try {
                if (!rmdir(WRITEPATH . 'media/' . $path . $dir)) {
                    return response()->setStatusCode(500)
                        ->setJSON([
                            'errors' => ['media_file' => lang('admin.media.dir_delete_error')]
                        ]);
                }

                return response()->setJSON([
                    'message' => lang('admin.media.dir_delete_success')
                ]);
            } catch (\Exception $exception) {
                return response()
                    ->setStatusCode(500)
                    ->setJSON([
                        'errors' => ['media_file' => lang('admin.media.dir_not_empty')]
                    ]);
            }
        }
        ...
        if ($this->request->getMethod() === 'POST') {
            ...
            if (empty($name)) {
                return response()
                    ->setStatusCode(422)
                    ->setJSON([
                        'errors' => [lang('admin.media.create_dir_error')]
                    ]);
            }
            ...
            try {
                if (mkdir(WRITEPATH . 'media/' . $path . $name, 0755)) {
                    return response()->setJSON([
                        'message' => lang('admin.media.create_dir_success')
                    ]);
                } else {
                    return response()
                        ->setStatusCode(500)
    ...
    public function ckbrowse()
    {
        ...
        return view(
            ...
            [
                'files' => $files,
                'section' => lang('admin.media.browse_files')
            ]
        );
    }
